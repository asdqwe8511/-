...
tools/test-trading.js   지표 공식·CSV 읽기·기법 목록·패턴 인식 점검 (282가지)
tools/test-trading-browser.js  차트 기법 화면 점검 (69가지, playwright 필요)
...
| 그룹 | 기법 | 표시 |
|---|---|---|
| 패턴 · 매매 전략 | 적삼병·흑삼병, 장악형, 망치형·유성형, 샛별형·저녁별형, 컵앤핸들, 쌍바닥·쌍봉, 헤드앤숄더(정·역), 볼린저 밴드 매매(되돌림·스퀴즈 돌파), 박스권 돌파, 이동평균 눌림목 | 패턴 구간을 옅게 칠하고 윤곽선을 긋는다. 완성된 봉에 ▲▼ 진입, 그 뒤로 목표가·손절가 선. 실제로 어느 쪽에 먼저 닿았는지 따라가 ★ 목표 도달 / ✕ 손절 / ○ 기간 만료 표시를 그 봉에 남기고, 목록의 "결과" 칸과 목표 도달률로 정리 |
...
| 기법 | 진입 | 목표가 | 손절가 |
|---|---|---|---|
| 적삼병 / 흑삼병 | 계단식 양봉(음봉) 3개, 시가가 전 봉 몸통 안 | 진입가 ± 세 봉 높이 | 세 봉의 최저(최고)가 |
| 장악형 | 오늘 몸통이 어제 몸통을 감쌈 | 손절폭 × 손익비 | 오늘 봉 반대쪽 끝 |
| 망치형 / 유성형 | 10일선 아래(위)에서 꼬리가 몸통의 n배 | 손절폭 × 손익비 | 꼬리 끝 |
| 샛별형 / 저녁별형 | 큰 봉 → 작은 봉 → 반대 방향 큰 봉이 첫 봉 절반 회복 | 손절폭 × 손익비 | 세 봉의 끝 |
| 컵앤핸들 | 둥근 바닥(깊이 n% 이상, 바닥이 가운데) + 손잡이(컵 위쪽 절반) 뒤 가장자리 돌파 | 돌파가 + 컵 깊이 | 손잡이 저점 |
| 쌍바닥 / 쌍봉 | 바닥(꼭대기) 둘이 허용 % 안, 사이 고점(저점) 돌파 | 목선 ± 패턴 높이 | 두 번째 바닥(꼭대기) |
| 헤드앤숄더 | 어깨-머리-어깨, 두 저점을 이은 목선 이탈 | 목선 − 머리 높이 | 오른쪽 어깨 |
| 볼린저 되돌림 | 밴드 밖에 있다가 안으로 들어온 봉 | 중심선 | 밖에 있던 동안의 극값 |
| 볼린저 스퀴즈 돌파 | 밴드폭이 최근 최저 수준일 때 상단(하단) 돌파 | 손절폭 × 2 | 중심선 |
| 박스권 돌파 | n봉 박스(폭 n% 이내)를 종가가 벗어남 | 박스 높이만큼 | 박스 가운데 |
| 이동평균 눌림목 | 장기선 위 상승 추세, 단기선까지 눌렸다가 양봉 | 손절폭 × 손익비 | 단기선 2% 아래 |

결과는 진입 다음 봉부터 "보유 봉수" 안에서 목표가·손절가 중 어느 쪽에 먼저 닿는지로 정합니다.
같은 봉에서 둘 다 닿으면 손절로, 갭으로 건너뛰면 시가로 체결된 것으로 봅니다.
수수료·슬리피지는 없고, 과거 봉을 따라간 것일 뿐 앞으로도 통한다는 뜻은 아닙니다.
...
```js
{
  id: 'my_rule', group: 'signal',            // pattern | trend | momentum | signal
...
파일 첫머리 주석에 overlay·panel·marker 의 모양이 전부 적혀 있습니다. 진입·목표·손절이 있는
전략은 `{index, side, text, entry, target, stop, horizon}` 목록을 `assembleTrades(bars, trades, color)` 에
넘기면 청산 시점 추적과 선·마커가 전부 만들어집니다(패턴 그룹의 기법들이 그렇게 되어 있습니다).
넣은 뒤에는 `node tools/test-trading.js` 가 새 항목의 출력 길이와 인덱스가 맞는지 자동으로 봅니다.

### 점검

```bash
node tools/test-trading.js            # 지표 공식(Wilder RSI 예제값 등)·CSV 읽기·기법 목록·패턴 인식·목표/손절 따라가기
