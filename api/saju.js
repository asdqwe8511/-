...
function describe(r, input) {
  ...
  // 격국 — 이 사주가 무엇을 중심으로 도는가
  const pt = a.pattern;
  out.push(`[격국] ${pt.name} — 월지 ${pt.monthBranch}의 ${pt.stem}(${pt.stemHanja})` +
    `${SajuEngine.josa(pt.stem, '이/가').slice(-1)} ${pt.via === '투간' ? '천간에 드러나' : '정기라'} ` +
    `${SajuEngine.josa(pt.god, '으로/로')} 잡았다. ${pt.plain}`);

  // 드문 구조 — "희귀하다"는 말을 근거 없이 하지 않으려고 백분율을 함께 준다.
  const rare = SajuEngine.findRareShapes(r.chart, a);
  out.push('[눈에 띄는 구조] ' + (rare.length
    ? rare.map((x) => `${x.name}(${x.why}) — 무작위 사주 100명 중 ${x.rarityPercent}명`).join(' / ')
    : '특별히 드문 배치는 없음'));
  out.push('  ※ 백분율이 큰 것은 흔한 배치다. 흔한 것을 "귀한 사주"라고 하지 말 것.' +
    ' 10% 아래일 때만 드물다고 말하고, 40%를 넘으면 "흔한 일"이라고 밝힐 것.');
  ...
  // 십신 그룹의 상태 — 개수만으로는 "있는데 못 쓴다"를 말할 수 없다.
  const gg = SajuEngine.godGroupStatus(r.chart, a);
  out.push('[다섯 갈래의 상태]');
  Object.keys(gg).forEach((k) => {
    const G = gg[k];
    out.push(`  ${G.label}(${G.means}): ${G.state}, 양 ${G.amount}` +
      ` / 천간에 ${G.revealed.length ? G.revealed.join('·') : '없음'}` +
      ` / 뿌리 ${G.rooted.length ? G.rooted.join('·') : '없음'}` +
      (G.damagedRoots ? ` / 뿌리 ${G.damagedRoots}군데가 충·형으로 흔들림` : '') +
      ` → ${G.usable}`);
  });
  ...
  // 건강 — 진단이 아니라 "어느 쪽이 먼저 지치는가"
  try {
    const hp = SajuEngine.healthProfile(r);
    out.push('[몸 — 오행이 치우친 쪽]');
    if (hp.risks.length) {
      hp.risks.forEach((x) => {
        out.push(`  ${x.elem} ${x.side}(전체의 ${x.share}%) → ${x.organs}, ${x.parts}: ${x.sign}`);
      });
    } else out.push('  오행이 고르게 퍼져 특별히 한쪽으로 몰리지 않는다');
    out.push(`  기신 ${SajuEngine.josa(hp.gisinBody.elem, '이/가')} 드는 때 먼저 반응하는 곳:` +
      ` ${hp.gisinBody.organs} — ${hp.gisinBody.sign}`);
    if (hp.weakestSeason) out.push(`  특히 조심할 철: ${hp.weakestSeason}`);
    out.push(`  용신 ${hp.yongsinCare.elem}에 맞는 음식: ${hp.yongsinCare.food}` +
      ` / 움직임: ${hp.yongsinCare.move}`);
    out.push('  ※ 진단도 병명도 아니다. "무리가 가기 쉬운 쪽과 생활 습관"으로만 쓸 것.');
  } catch (e) { /* 건강 계산이 실패해도 나머지 풀이는 그대로 나가야 한다 */ }

  // 배우자 자리와 인연이 드는 해
  try {
    const sp = SajuEngine.spouseProfile(r);
    out.push(`[배우자 자리(일지)] ${sp.seat.branch}(${sp.seat.hanja}) — 나에게 ${sp.seat.god},` +
      ` 오행 ${sp.seat.elem}` + (sp.seat.gongmang ? ', 공망에 듦' : ''));
    out.push('  일지가 맺는 관계: ' + (sp.seatRelations.length
      ? sp.seatRelations.map((x) => `${x.pair} ${x.name}`).join(' / ') : '없음'));
    out.push(`  배우자로 보는 십신: ${sp.mateGod}(${sp.mateGroup.label}) — ${sp.mateGroup.state},` +
      ` ${sp.mateGroup.usable}`);
    out.push(`  곁에 두면 편한 기운: ${sp.idealElem} / 오래 두면 지치는 기운: ${sp.avoidElem}`);
    out.push('  인연이 움직이는 해: ' + (sp.years.length
      ? sp.years.map((x) => `${x.year}(${x.hanja}: ${x.why.join('·')})`).join(' / ') : '앞으로 12년 안에는 두드러진 해 없음'));
  } catch (e) { /* 배우자 계산이 실패해도 나머지는 그대로 */ }

  // 택일 — 이번 달과 다음 달, 일마다 맞는 날
  try {
    // 화면의 달력과 같은 구간(절기로 나눈 달)을 본다. 지나간 날은 뺀다.
    const now = new Date(Date.now() + 9 * 3600 * 1000);
    const Y = now.getUTCFullYear();
    const todayNum = Y * 10000 + (now.getUTCMonth() + 1) * 100 + now.getUTCDate();
    const ym2 = SajuEngine.yearMonths(r, Y);
    let cur = ym2.months[0];
    ym2.months.forEach((m2) => {
      const a2 = m2.fromDate.y * 10000 + m2.fromDate.m * 100 + m2.fromDate.d;
      const b2 = m2.toDate.y * 10000 + m2.toDate.m * 100 + m2.toDate.d;
      if (todayNum >= a2 && todayNum < b2) cur = m2;
    });
    const range = { from: cur.fromDate, to: cur.toDate };
    out.push(`[지금 절기 달(${cur.label})의 택일 — 내 사주 기준, 오늘 이후만]`);
    Object.keys(SajuEngine.ACTIVITIES).forEach((k) => {
      const d = SajuEngine.pickDays(r, cur.fromDate.y, cur.fromDate.m, k, 2, { range });
      if (!d) return;
      out.push(`  ${k}: 좋은 날 ` + d.good.map((x) =>
        `${x.month}/${x.day}(${x.ganji.hanja}${x.why.length ? ' — ' + x.why.join('·') : ''}` +
        `, ${x.hours.map((h) => h.label).join(' 또는 ')})`).join(', ') +
        ' / 피할 날 ' + d.bad.map((x) =>
        `${x.month}/${x.day}${x.warn.length ? '(' + x.warn.join('·') + ')' : ''}`).join(', '));
    });
    out.push('  ※ 그날 자체의 길흉이 아니라 이 사람의 사주에 견준 값이다. 다른 사람에게는 다른 날이 나온다.');
  } catch (e) { /* 택일 계산이 실패해도 나머지는 그대로 */ }
...
- **사주 용어를 그대로 던지지 마세요.** 이 글을 읽는 사람은 사주를 모릅니다.
  용어를 쓸 때는 반드시 쉬운 말을 앞에 두고 용어를 괄호에 넣습니다.
  예) "편인" ✗ → "남다른 배움(편인)" ○ / "일지 자형" ✗ → "가장 가까운 자리가 스스로를
  긁는 배치(일지 자형)" ○ / "공망" ✗ → "기운이 비는 자리(공망)" ○
  한 절에서 같은 용어를 두 번째 쓸 때부터는 쉬운 말만 써도 됩니다.
  십신은 이렇게 풉니다 — 비견=나와 같은 편, 겁재=몫을 나눌 사람, 식신=즐겁게 만드는 힘,
  상관=말과 재주, 편재=크게 굴리는 돈, 정재=차곡차곡 모은 돈, 편관=나를 밀어붙이는 힘,
  정관=지켜야 할 자리, 편인=남다른 배움, 정인=나를 받쳐 주는 손.
  지지 관계는 이렇게 풉니다 — 충=정면으로 부딪힘, 형=서로 긁고 다툼, 파=잡은 것이 깨짐,
  해=알게 모르게 어긋남, 원진=이유 없이 거슬림, 자형=스스로를 긁음,
  육합=둘이 꼭 맞물림, 삼합=셋이 한 팀이 됨, 방합=같은 계절끼리 뭉침.
- 한자를 쓸 때는 한글을 앞에 두고 한자를 괄호에 넣습니다. "庚午" ✗ → "경오(庚午)" ○
...
**격국과 구조** — 이 사주가 무엇을 중심으로 도는지(격국)를 먼저 한마디로 잡고,
왜 그렇게 잡았는지(월지의 어느 글자가 천간에 드러났는지)를 한 줄 답니다. 그다음
'눈에 띄는 구조'를 읽는데, **반드시 백분율을 근거로 삼습니다** — 100명 중 40명이
넘는 배치를 "귀하다/드물다"고 하면 안 되고, "흔한 일입니다"라고 밝혀야 합니다.
10명 아래일 때만 드물다고 말합니다. 드문 게 없으면 없다고 말합니다.
...
**다섯 갈래와 십신** — 비겁·식상·재성·관성·인성이 각각 어떤 상태인지 표로 한 번
보여 주고, 개수만 세지 말고 자료의 '드러났는가(천간)'와 '뿌리가 있는가(지지)'를
함께 읽습니다. **"재성이 있긴 한데 못 쓴다"** 같은 판단이 여기서 나옵니다 — 천간에
드러났는데 뿌리가 없으면 겉만 있고 힘이 없는 것이고, 뿌리는 있는데 안 드러나면
있어도 꺼내 쓰기 어려운 것입니다. 재성(돈)과 관성(자리)은 이 사람의 일과 직결되니
반드시 짚습니다.
이어서 넘치는 십신과 없다시피 한 십신을 짚습니다. 넘치는 십신은
...
**일과 돈** — '다섯 갈래의 상태'와 '용신 배속의 일의 결'을 이어서, 어떤 분야에서
힘을 쓸 수 있고 어떤 업종이 불리한지 구체적으로 씁니다. 재성이 얇으면 월급처럼
꾸준한 쪽이 맞는지, 편재가 강하면 벌리는 쪽이 맞는지를 근거와 함께 말합니다.
지금 대운에서 돈을 어떻게 다루면 좋은지, 무엇을 조심해야 하는지도 함께요.

**몸** — 자료의 '오행이 치우친 쪽'을 읽습니다. 넘치는 오행과 모자란 오행이 각각
어느 곳에 먼저 걸리는지, 기신이 드는 때 무엇이 반응하는지, 어느 철을 조심해야
하는지를 말하고, 음식·움직임·생활 습관으로 이어 줍니다. **진단도 병명도 수명도
말하지 않습니다.** "무리가 가기 쉬운 쪽과 생활 습관"까지입니다.

**사람과 인연** — 자료의 '배우자 자리(일지)'를 읽습니다. 그 자리가 나에게 어떤
십신인지, 충·합으로 흔들리는지, 공망인지가 배우자운의 뼈대입니다. 배우자로 보는
십신(남자는 재성, 여자는 관성)의 상태를 함께 보고, 곁에 두면 편한 기운과 오래
두면 지치는 기운으로 어떤 사람이 맞고 어떤 사람이 힘든지 말합니다. 인연이
움직이는 해도 자료에 있는 연도만 씁니다. 가족·동료 관계에서 되풀이되는 패턴도
한 문단 넣습니다.

**이번 달, 이렇게 해보세요** — 두 가지를 한 절에 담습니다.
하나는 날 고르기 — 자료의 '일별 택일'을 다 옮기지 말고 이 사람이 지금 궁금해하는
것과 이어서 두세 가지만 고릅니다. 날짜와 시간대를 함께 말하고, 왜 그날인지(어떤
십신이 드는지)와 왜 그날을 피하는지를 한 줄씩 답니다. **그날 자체의 길흉이 아니라
이 사람의 사주에 견준 값**이라는 것을 한 번은 밝힙니다.
다른 하나는 개운 — 자료의 '용신 배속'을 말로만 옮기지 말고 바로 할 수 있는 형태로
바꿉니다. 책상을 어느 쪽으로 두는지, 어떤 색 옷과 소품을 쓰는지, 어디에 자주
가는지처럼요. 아침·낮·저녁 하루 루틴과 주마다 볼 체크리스트를 목록으로 만들고,
우선순위가 높은 행동 세 가지를 고릅니다. 한 달 뒤의 변화는 부풀리지 말고 현실적인
범위로만 말합니다.
...
## 언제가 좋고 언제를 조심할까
## 이번 달, 이렇게 해보세요
'다섯 갈래와 십신' 절에 표 하나, '12운성' 절에 대운별 단계 표 하나를 넣습니다.
'이번 달' 절은 목록으로 씁니다. 나머지는 문장입니다.

분량을 지키세요. 절이 많으니 각 절은 **3~5문장**으로 짧고 단단하게 씁니다.
