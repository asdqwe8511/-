...
1. **[체크박스 지정]** — 처음 한 번만. 반입내역 전체선택 체크박스는 화면에 id 가 없어서
   ...
   이 상자 안은 집지 않고, 체크박스가 아닌 것도 집지 않는다. 잘못 집느니 계속
   기다리는 편이 낫다 — 한 번은 [체크박스 지정] 버튼 자기 자신이 저장돼서,
   자동화가 5단계에서 제 버튼을 계속 눌러댄 일이 있었다.
2. **[점검]** — 화면을 바꾸지 않고 필요한 자리를 다 찾을 수 있는지만 봅니다.
3. **[● 대기 시작]** — 이제 화면의 **[박스번호채번]** 을 누르면 2~6단계가 이어서 돕니다.
   한 바퀴 돌고 반품예정번호에 커서가 오면 멈추고 다시 대기합니다.
   다음 건은 채번만 누르면 됩니다.

방아쇠는 클릭이 아니라 **박스번호 칸에 새 번호가 찍히는 것**이다. 클릭을 잡으려 들면
화면이 이벤트를 어떻게 흘리는지에 기대게 되는데, 번호는 어떤 경로로 눌렸든 그 칸에 들어온다.

상자 위 상태줄이 지금 무엇을 보고 있는지 말한다 — `대기 중 — 박스번호 칸: (비어 있음)`.
채번을 눌렀는데도 계속 비어 있다고 나오면 `CFG.s1.boxNo` 가 실제 칸이 아니라는 뜻이다.
